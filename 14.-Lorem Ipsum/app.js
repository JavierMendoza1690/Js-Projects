...
form.addEventListener("submit", (e) => {
  ...
  const value = parseInt(amount.value); //transforming amount.value to integer
  const random = Math.floor(Math.random()*text.length)
 

  if (isNaN(value) || value < 0 || value > 9) {
    result.innerHTML = `<p class="result"><p>${text[random]} `;
  }else{
    let tempText = text.slice(0,value);
   // console.log(tempText); //add the range of positions in a variable
